...
var setupAuthorization = function setupAuthorization(_ref) {
  var accessRules = _ref.accessRules;
  // accessRules:
  // [ { re: "company-org-chart.png", groups: [ { name: "foo@acme.com", "id": "groups/04iylrwe113ay6x" },... ] }, ...]
  accessRules.forEach(function (ar) {
    try {
      ar.re = new RegExp(ar.re);
    } catch (e) {
      // add some more context
      throw new Error("Could not process regular expression '".concat(ar.re, "' for access rules. (").concat(e.message, ")"));
    }
  });
  return {
    verifyAuthorization: function () {
      var _verifyAuthorization = _asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee(_ref2) {
        var user, path, _ref3, groups, cloudidentity, auth, authClient, _iterator, _step, _res$data, groupId, res;

        return regenerator.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                user = _ref2.user, path = _ref2.path;
      ...
      function verifyAuthorization(_x) {
        return _verifyAuthorization.apply(this, arguments);
      }
...
var accessAuthorizations = bucket.file('_access-authorizations.json');

var _await$accessAuthoriz = await accessAuthorizations.exists(),
    _await$accessAuthoriz2 = _slicedToArray(_await$accessAuthoriz, 1),
    accessAuthorizationsExist = _await$accessAuthoriz2[0]; // TODO: move to lib
...
if (accessAuthorizationsExist) {
  var fileStream = accessAuthorizations.createReadStream();
  var accessAuthStr = await streamToString(fileStream);
  var accessRules = JSON.parse(accessAuthStr);
  authorizer = setupAuthorization({
    accessRules: accessRules
  });
} else {
  console.log("No '_access-authorizations.json' file found.");
  authorizer = {
    verifyAuthorization: function verifyAuthorization() {
      return true;
    }
  };
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            ...
            case 11:
              ...
              _context3.next = 15;
              return authorizer.verifyAuthorization({
                user: userEmail,
                path: path
              });
